...
# %%
pd.read_csv("TaskB_OOF_Scores.csv")
...
#     "lora-3-fold-stratified-cv-flan-t5-large-lora",
    "lora-3-fold-stratified-cv-biobart-v2-large-peft-int8"
]
...
def main(filepath):
    ...
    test_df.rename(mapper={"ID":"TestID"}, \
                    axis=1, \
                    inplace=True)
    test_df.to_csv("taskB_SuryaKiran_run1_mediqaSum.csv",index=False)
#     task{A|B|C}_teamName_run{1|2|3}_mediqaSum.csv
